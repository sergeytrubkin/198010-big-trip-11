...
// фильтр
const createTripMainFilterTemplate = () => `
...
// создает секцию - дополнительные детали,
// описывающие указанную точку маршрута
const createEventDetailsTemplate = () => `
  <section class="event__details"></section>`;

// предложения в указанной точке маршрута
const createEventDetailsOffersTemplate = () => `
...
const createEventDetailsDestinationTemplate = () => `
  <section class="event__section  event__section--destination">
    ...
    <p class="event__destination-description">Geneva is a city in Switzerland that lies at the southern tip of expansive
      Lac Léman (Lake Geneva). Surrounded by the Alps and Jura mountains, the city has views of dramatic Mont Blanc.</p>
...
// содержание одного дня маршрута
const createDayContentTemplate = () => `
...
render(tripInfo, createInfoMainTemplate(), `beforeend`);
...
for (let i = 0; i < DAY_COUNT; i++) {
  ...
  for (let j = 0; j < POINT_COUNT; j++) {
    render(tripEventsList, createTripPointTemplate(), `afterbegin`);

    const eventOffersList = tripEventsList.querySelector(`.event__selected-offers`);

    for (let k = 0; k < OFFER_COUNT; k++) {
      render(eventOffersList, createEventOfferTemplate(), `beforeend`);
    }
  }
}
